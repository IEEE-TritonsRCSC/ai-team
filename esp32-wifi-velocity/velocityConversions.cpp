...
void action_to_byte_array(array<uint8_t, 8>& wheel_speeds_byte) {
    ...
    getVelocityArray(wheel_speeds, 0.0, 0.1, 0.1, 0.1);
    valuesToBytes(wheel_speeds, wheel_speeds_byte);
}

void action_to_byte_array_with_params(array<uint8_t, 8>& wheel_speeds_byte,
                                      double vx, double vy, double rotV) {
    array<int, 4> wheel_speeds;

    // Use the actual parameters instead of hardcoded values
    getVelocityArray(wheel_speeds, 0.0, vx, vy, rotV);
