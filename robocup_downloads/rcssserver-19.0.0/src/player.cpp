...
Player::goalieCatch( double dir )
{
    ...
    if ( success )
    {
        // Preserve the stock goalie catch correction, but do not teleport or
        // rotate a field player. Field-player catches are used by the ssim
        // dribble glue; their pose must remain the result of explicit dash/turn
        // commands so simulation does not provide motion unavailable to a real
        // robot. Stadium::ballCaught() still establishes ownership and
        // Stadium::incMovableObjects() keeps the caught ball in front.
        if ( this->isGoalie() )
        {
            PVector new_pos = M_stadium.ball().pos() - this->pos();
            double mag = new_pos.r();
...
Player::ballKickableInFrontReceptionCone() const
{
    ...
    // Mechanical reception angle for the ball center. The wider CAD angle is
    // measured at the outside of the ball radius and should not be used here.
