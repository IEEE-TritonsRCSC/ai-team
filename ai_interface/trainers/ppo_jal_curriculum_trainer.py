...
def _accumulate_active_primitives(
    counter: Counter,
    primitive_indices,
    agent_active_mask,
) -> None:
    """Count sampled primitives for active agents, excluding a_max padding."""

    primitives = np.asarray(primitive_indices).reshape(-1)
    active = (
        np.asarray(agent_active_mask).reshape(-1)
        if agent_active_mask is not None
        else np.ones(primitives.shape, dtype=np.float32)
    )
    if active.size != primitives.size:
        raise ValueError(
            "agent_active_mask and primitive_indices must have equal flattened size"
        )
    for primitive, is_active in zip(primitives, active):
        if float(is_active) > 0.5:
            counter[PRIMITIVE_NAMES[int(primitive)]] += 1


def _mask_latched_dribble_params(env: JALTeamEnv, transition: Dict[str, Any]) -> None:
    """Remove PPO credit for parameters ignored by an already-latched macro."""
    param_masks = transition.get("param_active_mask")
    param_logprobs = transition.get("param_logprob")
    if param_masks is None or param_logprobs is None:
        return
    for slot, rid in enumerate(env.robot_ids):
        state = env.dribble_states.get(rid)
        if state is not None and state.committed:
            param_masks[slot, :] = 0.0
            param_logprobs[slot] = 0.0
...
class PPOJALCurriculumTrainer(BaseTrainer):
    ...
    def _build_env(
        ...
        return JALTeamEnv(
            ...
            turn_stall_limit=int(_stage_or_top("turn_stall_limit", 12)),
            turn_stall_displacement=float(
                _stage_or_top("turn_stall_displacement", 0.05)
            ),
            kick_requires_aim=bool(_stage_or_top("kick_requires_aim", False)),
    ...
    def setup_model(self, env: JALTeamEnv, num_robots: int):
        ...
        for k in [
            ...
            "param_ent_coef", "param_log_std_init",
    ...
    def _training_loop(
        ...
        while steps_done < total_timesteps:
            ...
            _mask_latched_dribble_params(self.env, transition)

            # Accumulate requested primitives for active robots only. PPO emits
            # a_max slots, so unfiltered counting includes inactive padding.
            _accumulate_active_primitives(
                win_actions, action["primitive_idx"], agent_mask
            )
