...
class SimpleInterceptPlayer(Player):
    ...
    def hybrid_capture(self, self_p0, heading, ball_vel_est, ball_pos, theta,
                       rect_bounds=None, game_state=None, margin = 0.3):
...
class SimpleInterceptDemoAI(SoccerAI):
    def __init__(self, team_info):
        ...
        self.shooter_attempt_kick_buffer = 15
        self.receiver_attempt_kick_buffer = 15
        self.shooter = SimpleInterceptPlayer(decay=self.decay, ball_decay=self.ball_decay, is_goalie=False)
        self.receiver = SimpleInterceptPlayer(decay=self.decay, ball_decay=self.ball_decay, is_goalie=True)
    ...
    def _action_shooter(self, self_p0, heading, ball_vel_est, ball_pos, player: SimpleInterceptPlayer,
                        game_state=None):
        ...
        ball = np.array(ball_pos[:2], dtype=float)
        to_ball = ball - self_p0
        desired_theta = math.atan2(GOAL_L[1] - ball[1], GOAL_L[0] - ball[0])
        to_goal = GOAL_L - self_p0
        
        if self.shooter_attempt_kick_buffer >= 15:
            if np.arccos(np.dot(to_ball, to_goal)/np.linalg.norm(to_goal)/np.linalg.norm(to_goal)) < 0.35:
                margin = 0.1
            else:
                margin = 0.3
        ...
        self.shooter_attempt_kick_buffer += 1
        
        if np.linalg.norm(ball_vel_est) >= 0.5:
    ...
    def _action_receiver(self, self_p0, heading, ball_vel_est, ball_pos, player: SimpleInterceptPlayer,
        ...
        # Try to be in position to block ball
        ...
        if self.receiver_attempt_kick_buffer >= 15:
            if ball_speed > 0.05:
                cmd = player.goto(ball_pos[0], ball_pos[1], [self_p0[0], self_p0[1], heading], game_state, theta=kick_angle, margin= 0.1)
            else:
                cmd = player.hybrid_capture(
                    self_p0,
                    heading,
        ...
        if self.receiver_attempt_kick_buffer % 2 == 0:
            cmd = f"kick {100} {0}"
        else:
            cmd = f"dash {50} {0}"
        
        self.receiver_attempt_kick_buffer += 1
